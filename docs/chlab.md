...
The `chlab_*` MCP tools use the local root service at `/run/aviary-chlab.sock`. The Aviary server does not need Docker socket access. Build the service as an ordinary user, then install the finished binary as root:
...
Run it with a system service that executes only the root-owned binary. For example, save this unit as `/etc/systemd/system/aviary-chlab.service`, replacing `AVIARY_SERVER_GROUP` with the group of the Aviary server process:

```ini
[Unit]
Description=Aviary ClickHouse verification lab
...
Type=simple
User=root
Group=AVIARY_SERVER_GROUP
WorkingDirectory=/
ExecStart=/usr/local/bin/aviary-chlab chlab-service
Restart=on-failure
RestartSec=2
UMask=0077
...
Run `sudo systemctl daemon-reload` and `sudo systemctl enable --now aviary-chlab.service`. The service never compiles or reads the user checkout at startup. Rebuild and reinstall the binary before restarting the service for future updates.
...
For an agent using runtime labs, allow `chlab_start`, `chlab_status`, `chlab_query`, `chlab_stop`, and the three `chlab_node_*` tools. If it also uses `exec` for source questions, restrict that permission to a bounded, read-only source helper. Agent rules should permit `chlab_start` only when a user explicitly requests runtime verification. The agent should label SQL results as observed in the reported `SELECT version()` build and image digest, and label source conclusions separately.
